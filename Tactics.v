...
(* 当apply一个implication: P -> Q的时候，会把P加入goal *)
...
Proof.
  ...
  transitivity [c;d].
  apply eq1. apply eq2.   Qed.
(* transitivity告诉我们只要证明A=B,B=C就能证明A=C *)
...
Proof.
  ...
  rewrite <- eq12 in eq2.
  injection eq2 as eq21.
  symmetry in eq21.
  transitivity z.
  apply eq11. apply eq21.
  (* FILL IN HERE *) Qed.
(** [] *)
...
Proof.
  ...
  - simpl. reflexivity.
  - intros n. intros H. simpl in H. destruct n as [| n'].
    + discriminate H.
    + injection H as H'. apply IHl'. apply H'.
  (* FILL IN HERE *) Qed.
(** [] *)
...
Proof.
  ...
  - intros l1 l2. unfold split. intros H. injection H as H1 H2. 
    symmetry in H1. symmetry in H2. rewrite -> H1. rewrite -> H2. 
    unfold combine. reflexivity.
  - intros l1 l2. intros H.
    simpl in H.
    destruct (split t) as [lx ly] eqn:eq1.
    injection H as H1 H2. symmetry in H1. symmetry in H2. 
...
Proof.
  ...
  induction n as [| n' IHn'].
  - intros m p. intros H1 H2. destruct m as [| m'].
    + apply H2.
    + simpl in H1. discriminate H1.
  - intros m p. destruct m as [| m'].
    + intros H1. simpl in H1. discriminate H1.
    + destruct p as [| p'].
      * intros H1 H2. simpl in H2. discriminate H2.
      * simpl. apply IHn'.
  (* FILL IN HERE *) Qed.
(** [] *)
