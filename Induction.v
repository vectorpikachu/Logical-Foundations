...
Lemma mul_helper : forall n k : nat, 
  n * (S k) = n + n * k.
Proof.
  intros n k.
  induction n as [| n' IHn'].
...
Proof.
  ...
  - simpl. rewrite -> mul_helper. rewrite -> IHn'. reflexivity.
  (* FILL IN HERE *) Qed.
(** [] *)
...
Fixpoint bin_to_nat (m:bin) : nat :=
  match m with
  | Z => 0
  | B0 m0 => (bin_to_nat m0) * 2
  | B1 m1 => (bin_to_nat m1) * 2 + 1
  end.
  (* REPLACE THIS LINE WITH ":= _your_definition_ ." *)
...
Lemma x_2_1 : forall n : nat, n * 2 + 1 = S (n * 2).
...
Proof.
  ...
  - (* b = Z *)
    simpl. reflexivity.
  - simpl. rewrite -> x_2_1. reflexivity.
  - simpl. rewrite -> IHb''. rewrite -> x_2_1. simpl. reflexivity.   
  (* FILL IN HERE *) Qed.
