...
Definition filter_even_gt7 (l : list nat) : list nat := 
  filter (fun x => andb (even x) (negb (x <=? 7))) l.
  (* REPLACE THIS LINE WITH ":= _your_definition_ ." *)
...
Lemma map_fenpei : forall (X Y : Type) (f : X -> Y) (l1 l2: list X),
...
induction l as [| h l' IHl'].
- simpl. reflexivity.
- simpl. rewrite -> map_fenpei. simpl. rewrite -> IHl'. reflexivity.  
  (* FILL IN HERE *) Qed.
(** [] *)
...
Proof.
  ...
  induction l as [| h l' IHl'].
  - simpl. reflexivity.
  - assert (H:fold_length (h::l') = 1 + fold_length l'). reflexivity.
    rewrite -> H. rewrite -> IHl'. simpl. reflexivity.
(* FILL IN HERE *) Qed.
(** [] *)
...
Theorem nth_error_length : forall X l n,
  length l = n -> @nth_error X l n = None.
Proof.
  intros X l.
  induction l as [| h l' IHl']. 
  - intros n H. simpl. reflexivity.
  - intros n H. simpl. simpl in H. rewrite <- H. rewrite -> IHl'. 
...
:= fun (X : Type)(f: X -> X) (x: X) => f (n X f x).
  (* REPLACE THIS LINE WITH ":= _your_definition_ ." *)
(* 这里n X f x表示f^n x，再套上一个f后就变成了f^(n+1) x, fun X f x => f^(n+1) x*)
...
Proof. 
assert (H: plus (plus two two) three = fun (X : Type) (x : X -> X) (x0 : X) =>
       x (x (x (x (x (x (x x0))))))). reflexivity.
assert (G: plus one (plus three three) = fun (X : Type) (x : X -> X) (x0 : X) =>
       x (x (x (x (x (x (x x0))))))). reflexivity.
rewrite -> H. rewrite -> G. reflexivity.
(* FILL IN HERE *) Qed.
...
Definition mult (n m : cnat) : cnat :=
  fun (X : Type) (f: X -> X) (x: X) => n X (fun (y : X) => m X f y) x.
  (* REPLACE THIS LINE WITH ":= _your_definition_ ." *)
...
Definition exp (n m : cnat) : cnat :=
  ...
  (*关键是m个n相乘 m (cnat -> cnat) (mult n) one*)
  (* 我不太理解这个定义的 *)
  (* REPLACE THIS LINE WITH ":= _your_definition_ ." *)
...
Proof. assert (H: exp two two = fun (X : Type) (x : X -> X) (x0 : X) =>
       x (x (x (x x0)))). reflexivity.
assert (G: plus two two = fun (X : Type) (x : X -> X) (x0 : X) =>
       x (x (x (x x0)))). reflexivity.
rewrite -> G. rewrite -> H. reflexivity.
(* FILL IN HERE *) Qed.
