...
Proof.
  intros n m.
  intros H.
  apply conj. (* 要证明的结论被分为两个 *)
  destruct n as [| n']. (* 现在有三个要证明的，所以分成三个 *)
  - reflexivity. 
  - discriminate H.
  - destruct m as [|m'].
    + reflexivity.
...
Proof.
  (* WORKED IN CLASS *)
  intros P contra.
  destruct contra.  Qed. (* 如果是一个False，可以直接destruct来完成任何目标 *)
...
Proof.
  ...
  intros HQ.
  intros HP.
  apply HQ. (* 要证明False,现在应用Q->False，那么要证明Q *)
  apply H. (* 要证明Q，现在应用P->Q，那么要证明P *)
  apply HP.
  (* FILL IN HERE *) Qed.
(** [] *)
...
Proof.
  ...
  split.
  - intros HPQR. split.
    + destruct HPQR as [HP | [HQ HR] ]. (* 这一步讨论在HP 或者 HQ/\HR的条件下证明 *)
      * left. apply HP.
      * right. apply HQ.
    + destruct HPQR as [HP | [HQ HR] ].
      * left. apply HP.
      * right. apply HR.
  - intros HPQR. destruct HPQR as [[HP | HQ] [HP'| HR] ].
    + left. apply HP.
    + left. apply HP.
    + left. apply HP'.
    + right. split.
...
Proof.
  intros n m p.
  rewrite mul_eq_0. rewrite mul_eq_0. rewrite or_assoc. (* or是右结合的 *)
  reflexivity.
Qed.
...
Proof.
  intros X P Q.
  split.
  - intros HPQ. destruct HPQ as [x0 EPQ].
    destruct EPQ as [EP |EQ].
    + left. exists x0. apply EP.
    + right. exists x0. apply EQ.
  - intros HPQ. destruct HPQ as [ HP| HQ ].
    + destruct HP as [x0 EP]. exists x0. left. apply EP.
    + destruct HQ as [x0 EQ]. exists x0. right. apply EQ.
   (* FILL IN HERE *) Qed.
(** [] *)
...
Proof.
  ...
  - intros n H. destruct n as [| n'].
    ...
    + simpl in H. apply IHm' in H. destruct H as [x0 E]. exists x0.
      rewrite -> add_comm. rewrite <- plus_n_Sm. f_equal. 
...
Proof.
  ...
  - intros m x E. simpl. reflexivity.
  - intros m x E. destruct x as [| x'].
    + rewrite -> add_0_r in E. rewrite <- E. apply leb_refl.
    + destruct m as [| m'].
      * simpl in E. discriminate E.
      * simpl in E. injection E as E'. simpl. apply IHn' with (x := S x'). apply E'.
  (* FILL IN HERE *) Qed.
  ...
  { induction l as [|x l' IHl'].
    ...
    - simpl. intros H. destruct H as [H1 | H2].
      + exists x. simpl. split.
        * apply H1.
        * left. reflexivity.
      + apply IHl' in H2. destruct H2 as [x' E]. exists x'. destruct E as [E1 E2].
        split.
        * apply E1.
        * right. apply E2.
  }
  { induction l as [|x l' IHl'].
    - intros contra. simpl in contra. destruct contra. destruct H. destruct H0.
    - simpl. intros H. destruct H as [x' E]. destruct E as [ E1 E2].
      destruct E2 as [E21 | E22].
      + rewrite <- E21 in E1. left. apply E1.
...
Proof.
  ...
  - simpl. assert (H: forall P, P <-> False \/ P). { split. 
    + intros HP. right. apply HP.
    + intros G. destruct G as [G1 | G2].
      * destruct G1.
      * apply G2. }
    intros l' a.
    apply H with (P := (In a l')).
  - simpl. intros l'0 a. split.
    + intros H. destruct H as [H1 | H2].
      * left. left. apply H1.
      * rewrite <- or_assoc. right. apply IH. apply H2.
    + intros H. rewrite <- or_assoc in H. destruct H as [H1 | H2].
      * left. apply H1.
...
Proof.
  ...
  - induction l as [| x l' IHl'].
    + simpl. intros H. intros x H'. destruct H'.
    + simpl. intros H. intros x0 H'. destruct H' as [H1 | H2].
      * rewrite <- H1. apply H.
...
Proof.
  ...
  - rewrite -> even_S. destruct (even n') eqn:Eq.
    + simpl. destruct IHn' as [k H]. exists k. f_equal. apply H.
    + simpl. destruct IHn' as [k H]. exists (k+1). rewrite -> add_comm. 
      rewrite -> double_incr. f_equal. apply H.
...
Proof.
  ...
  - destruct b1 eqn:Eq1.
    + destruct b2 eqn:Eq2.
      * simpl. intros H. split. apply H. apply H.
      * simpl. intros H. discriminate H.
    + destruct b2 eqn:Eq2.
      * simpl. intros H. discriminate H.
      * simpl. intros H. discriminate H.
  - intros H. destruct H as [H1 H2]. rewrite -> H1. rewrite -> H2. simpl. reflexivity.
  (* FILL IN HERE *) Qed.
...
Proof.
  ...
  - intros H. destruct H as [H1 | H2].
    ...
    + rewrite H2. simpl. destruct b1. simpl. reflexivity. simpl. reflexivity.
  (* FILL IN HERE *) Qed.
(** [] *)
...
Proof.
  intros x y. split.
  - intros H. unfold not. intros H'. rewrite -> H' in H. rewrite -> eqb_refl in H.
    discriminate H.
  - unfold not. intros H. destruct (x =? y) eqn:E.
    + rewrite -> eqb_eq in E. apply H in E. destruct E.
    + reflexivity.
  (* FILL IN HERE *) Qed.
(** [] *)
...
Proof.
  ...
  - generalize dependent l2.
    ...
    + intros l2. destruct l2 as [| x2 l2'].
      ...
      * simpl. intros H'. discriminate H'.
    + intros l2. simpl. destruct l2 as [| x2 l2'].
      * intros H'. discriminate H'.
      * intros H'. apply andb_true_iff in H'. destruct H' as [H1 H2].
        apply H in H1. apply IHl1' in H2. rewrite -> H1. rewrite -> H2. reflexivity.
  - generalize dependent l2.
    induction l1 as [| x1 l1' IHl1'].
    + destruct l2 as [| x2 l2'].
      ...
      * intros H'. discriminate H'.
    + destruct l2 as [| x2 l2'].
      * intros H'. symmetry in H'. discriminate H'.
      * intros H'. injection H' as H1 H2. simpl. apply IHl1' in H2. apply H in H1.
...
Proof.
  ...
  - induction l as [| x l' IHl'].
    + simpl. intros H. reflexivity.
    + simpl. intros H. apply andb_true_iff in H. destruct H as [H1 H2].
      apply IHl' in H2. split. apply H1. apply H2.
  - induction l as [|x l' IHl'].
    + simpl. intros H. reflexivity.
    + simpl. intros H. destruct H as [H1 H2]. apply IHl' in H2.
...
Proof.
  ...
  - simpl. reflexivity.
  - simpl.  intros l1. apply (IHl' (h::l1)).
Qed.
...
Proof.
  ...
  - simpl. reflexivity.
  - simpl. rewrite <- IH. rewrite <- rev_append_app'. simpl. reflexivity.
(* FILL IN HERE *) Qed.
(** [] *)
...
Proof.
  intros H. intros X P. intros G x.
  specialize (H (P x)).
  destruct H as [H1 | H2].
  - apply H1.
  - (* H2 G矛盾了*) exfalso. apply G. exists x. apply H2.
  (* FILL IN HERE *) Qed.
(** [] *)
