...
Proof.
  ...
  rewrite <- snd_fst_is_swap. simpl. reflexivity.
  (* FILL IN HERE *) Qed.
(** [] *)
...
Lemma EQUAL : forall n : nat, n =? n = true.
Proof. intros n.
...
Proof.
  intros v s.
  destruct s as [| h t].
  - simpl. rewrite -> EQUAL. reflexivity.
  - simpl. rewrite -> EQUAL.
    destruct (h =? v) eqn:E.
    + reflexivity.
    + reflexivity.
...
Fixpoint eqblist (l1 l2 : natlist) : bool :=
  ...
  | h1::t1 => match l2 with
              | nil => false
              | h2::t2 => if h1 =? h2 then eqblist t1 t2 else false
              end
  end.
  (* REPLACE THIS LINE WITH ":= _your_definition_ ." *)
...
Proof.
  intros l. induction l as [| h l' IHl'].
  - simpl. reflexivity.
  - simpl. rewrite -> EQUAL. rewrite <- IHl'. reflexivity.
  (* FILL IN HERE *) Qed.
(** [] *)
